...
sealed class PlayerMouseLookSystem : IEcsInitSystem, IEcsRunSystem
{
    ...
    public void Run()
    {
        foreach (var i in _mouseLookFilter)
        {
            ...
            var rotateX = Quaternion.AngleAxis(axisX, lookComponent.MouseSensitivity * Time.deltaTime * Vector3.up);
            var rotateY = Quaternion.AngleAxis(axisY, lookComponent.MouseSensitivity * Time.deltaTime * Vector3.right);
